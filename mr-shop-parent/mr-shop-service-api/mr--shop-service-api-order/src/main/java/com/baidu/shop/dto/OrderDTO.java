...
public class OrderDTO {
    ...
    @NotNull(message = "收货地址不能为空", groups = {MrOperation.Update.class})
    private Integer addrId;

    @ApiModelProperty(value = "支付方式" ,example = "1")
    @NotNull(message = "支付方式不能为空", groups = {MrOperation.Update.class})
    private Integer payType;

    @ApiModelProperty(value = "购买商品ID集合")
    @NotEmpty(message = "购买商品不能为空", groups = {MrOperation.Update.class})
